...
- `model.forestay.beforeUpdate` - We use this instead of the Saild beforeUpdate callback, because we want access to REQ,RES and saved ids.  The saved values are contained in `forestay.save` and the original ID is `forestay.id` if you need to look up pre-saved values in the database
```Javascript
