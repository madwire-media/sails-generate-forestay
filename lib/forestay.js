                ...
                updateCollectionsAndModels(forestay, collections, newCollections, models, function(err) {
                    ...
                    if (typeof _.get(forestay.config, ['forestay', 'afterUpdate']) === 'function') {
                      forestay.config.forestay.afterUpdate(req, res, forestay, function (err, forestay) {
                        if (err) return res.serverError(err)
                        return res.redirect(getReturnUrl(forestay))
                      })
                    }
                    else {
                        return res.redirect(getReturnUrl(forestay))
    ...
    async.eachOfSeries(forestay.config.attributes, function eachAttr(attr, i, cb) {
        if (typeof _.get(forestay.config.attributes[i], ['collection']) === 'string') {
            ...
            global[modelName].find().exec(function(err, r) {
                ...
                if (forestay.id) {
                  if (_.get(forestay.config.attributes[i], ['meta','forestay','hideInForm']) === true || _.get(forestay.config.attributes[i], ['meta','forestay','hideInIndex']) === true){
                    // do not return associated records
                        forestay.model.find({
                            where: {
                                id: forestay.id
                            }
                        ...
                        }).populate(_.get(forestay.config.attributes[i], ['model'])).exec(function(err, result) {
                            if (err) return cb(err)
                            forestay.config.attributes[i].selectedValue = result
                            return cb(null)
                        })
...
function getReturnUrl(forestay, cb){
