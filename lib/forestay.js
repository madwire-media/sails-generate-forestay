...
var forestayHelpers = {
    index: function(forestay, req, res) {
        ...
        forestay.model.count().exec(function countCB(error, total_records) {
            filter.limit = forestay.config.forestay.index.itemsPerPage;
            var number_of_pages = Math.ceil(total_records / forestay.config.forestay.index.itemsPerPage);
            var pg = 1;

            if (req.param('pg') && !isNaN(req.param('pg')) && req.param('pg') > 1) {
                pg = req.param('pg');
                if (pg > number_of_pages) {
                    pg = 1;
                } else if (pg <= number_of_pages) {
                    var skip = 0;
                    filter.skip = (pg - 1) * forestay.config.forestay.index.itemsPerPage;
                }
            }
