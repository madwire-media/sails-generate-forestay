...
var forestayHelpers = {
  index: function (forestay, req, res) {
    ...
    var sort = "";
    if (typeof forestay.config.forestay.index.defaultSort != 'undefined' && typeof forestay.config.forestay.index.defaultSort.attribute != 'undefined') {
      var sort_method = "ASC";
      if (typeof forestay.config.forestay.index.defaultSort != 'undefined' && typeof forestay.config.forestay.index.defaultSort.method != 'undefined') {
    ...
    var query = forestay.model.find(filter).limit(forestay.config.forestay.index.itemsPerPage);
    if(skip > 0) {
      query.skip(skip);
    }
    if(sort != "") {
  ...
  createUpdatePost: function (forestay, req, res) {
    ...
    Object.keys(forestay.config.attributes).forEach(function (attrKey) {
      ...
      if (typeof _.get(forestay.config.attributes[attrKey], ['model']) === 'string') {
        attributeType = 'model'
        if (req.body[attrKey] === '') req.body[attrKey] = null
        if (typeof req.body[attrKey] !== 'undefined') save[attrKey] = parseInt(req.body[attrKey])
        // models.push({
        //   key: attrKey,
