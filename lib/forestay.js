...
var forestayHelpers = {
    index: function(forestay, req, res) {
      ...
      if (_.get(forestay.config.forestay.index, ['filterLogicalOperator']) === 'or' && Object.keys(filter).length > 0) {
        ...
        if (_.get(forestay, ["config","forestay","hasFilters"]) === true) {
        var modelName, hideInIndex
        Object.keys(forestay.config.attributes).forEach(function(attrKey) {
            if(_.get(forestay.config.attributes[attrKey], ['model']) && _.get(forestay.config.attributes[attrKey], ['meta','forestay','filterable']) === true) {
            ...
            } else if(_.get(forestay.config.attributes[attrKey], ['collection']) && _.get(forestay.config.attributes[attrKey], ['meta','forestay','filterable']) === true) {
