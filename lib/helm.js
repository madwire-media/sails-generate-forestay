...
var view_base = path.join(__dirname,'..','views','helm') //"./views/helm";
...
var helm_helpers = {
  index: function(helm, req,res){
    helm.model.find({}, function(err, records){
      ...
      res.view(view_base + '/index.ejs', {layout:default_layout, helm, moment})
    })
  },
  createView: function(helm,req,res){
    res.view(view_base + '/createUpdate.ejs', {helm})
  },
  ...
  updateView: function(helm,req,res){
    helm.model.find({id: helm.id}).limit(1).exec(function(err, record){
      ...
      record = record[0]
      res.view(view_base + '/createUpdate.ejs', {layout: default_layout, helm, record})
